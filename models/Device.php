...
class Device
{
    ...
    public function create($userToken, $deviceName)
    {
        ...
        $stmt->bind_param("sis", $deviceName, $userId, $deviceToken);
    ...
    public function addCommandToQueue($userToken, $deviceId, $command, $info)
    {
        ...
        $sql =
            "INSERT INTO DeviceQueue (DeviceId, command, info, issuedOn) VALUES (?, ?, ?, NOW())";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param(
            "iss",
            $deviceId,
    ...
    public function executeCommandFromQueue($deviceToken)
    {
        ...
        if ($result->num_rows > 0) {
            ...
            $sql =
                "SELECT * FROM DeviceQueue WHERE DeviceId = ? AND executedOn IS NULL ORDER BY issuedOn ASC LIMIT 1";
            $stmt = $this->conn->prepare($sql);
            $stmt->bind_param("i", $deviceId);
            $stmt->execute();
            $result = $stmt->get_result();
    ...
    public function createSchedule($userToken, $deviceId, $command, $info, $time)
    {
        ...
        $sql =
            "INSERT INTO DeviceScheduler (DeviceId, command, info, time) VALUES (?, ?, ?, ?)";
    ...
    public function deleteSchedule($userToken, $deviceId, $scheduleId)
    {
        ...
        $stmt->bind_param("ii", $scheduleId, $deviceId);
    ...
    public function executeSchedule($deviceToken)
    {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($result->num_rows > 0) {
                ...
                $sql =
                    "UPDATE DeviceScheduler SET lastExecuted = NOW() WHERE id = ?";
                ...
                return [
                    "status" => "error",
                    "message" => "No scheduled commands found",
                ];
            }
        } else {
    ...
    {
        $sql =
            "SELECT UserId FROM Sessions WHERE token = ? AND expireOn > NOW()";
